...
class ConversationalRAG:
    def __init__(self, session_id: str, retriever= None):
        try:
            ...
            log.error("Initialization error in Conversational RAG", error=str(e))
    ...
    def load_retriever_from_faiss(self, index_path: str) -> FAISS:
        ...
        try:
            ...
            log.info("Retriever created from FAISS vectorstore", index_path=index_path)
    ...
    def invoke(self, user_input: str, chat_history: Optional[list[BaseMessage]] = None) -> str:
        try:
            ...
            log.info("RAG invocation successful", session_id=self.session_id)
            if not answer:
                log.info('No answer generated by RAG chain', user_input=user_input, session_id=self.session_id)
                return "No answer could be generated based on the provided documents."
            
            log.info('Answer generated by RAG chain', 
                            user_input=user_input,
                            answer=answer[:100],  # Log only first 100 chars
                            session_id=self.session_id)
            return answer

        except Exception as e:
            log.error("Error during RAG invocation", error=str(e))
